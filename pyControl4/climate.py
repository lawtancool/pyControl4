...
class C4Climate(C4Entity):
    ...
    async def getHVACModes(self):
        """Returns the power state of thee fan as a boolean (True=on, False=off)."""
        value = await self.director.getItemVariableValue(self.item_id, "HVAC_MODES_LIST")
        return value
    ...
    async def getCoolSetpointF(self):
        """Returns the power state of thee fan as a boolean (True=on, False=off)."""
        value = await self.director.getItemVariableValue(self.item_id, "COOL_SETPOINT_F")
        return value
    
    async def getHeatSetpointF(self):
        """Returns the power state of thee fan as a boolean (True=on, False=off)."""
        value = await self.director.getItemVariableValue(self.item_id, "HEAT_SETPOINT_F")
        return value
